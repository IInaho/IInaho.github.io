...
21. 不使用相同的内置指针值初始化或 reset 多个智能指针。
22. 不 delete get()返回的指针
23. 不使用 get 初始化或 reset 另一个指针。
24. 如果使用 get 返回的指针，当最后一个对应的智能指针销毁后，此指针失效
25. 如果使用智能指针管理的资源不是 new 分配的内存，记住传递给它一个删除器。
26. 某个时刻只能有一个 unique_ptr 指向一个给定对象，定义 unique_ptr 需要将其绑定到 new 返回的指针上，必须采用直接初始化。
27. unique_ptr 不支持普通的赋值和拷贝操作。
28. u.release 函数放弃 u 对指针的控制权，返回指针。u.reset 释放 u 指向的对象。
29. weak_ptr 是一种不控制所指向对象生存期的智能指针，指向由一个 shared_ptr 管理的对象。
